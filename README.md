...
- **Lattice button.** A button at the top right of the drawing area shows or hides the backing lattice strings in 2D and both 3D views, so you can see only the bulb cables. It stays in sync with the "lattice lines" checkbox.
- **Two 3D views.** "3D real" is the bundle model above: honeycomb bundles, crossing stacks, and ties squeezing the bundles. "3D stacked" is technical: every segment's cables sit one behind another in front of the lattice plane, 5.5 mm per cable, so how far a segment sticks out shows how many cables share it. Tubes are coloured by that count on a dark-purple-to-yellow scale that tops out at the 98th percentile, and a legend explains it. "Colour each cable" still overrides the colours. At the defaults the busiest segment carries 23 cables and sticks out 12.7 cm.
